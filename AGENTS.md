...
A single-page, fully client-side invoice generator for VOM Industry Group. No backend, no database, no auth. Everything runs in the browser. Deployed as a static site on Netlify.

## Key Architecture Notes

### Single Component Architecture
All invoice state lives in `src/components/InvoiceGenerator.tsx` via `useState`. The `InvoiceData` type describes the full invoice model. `InvoicePreview` (white A4 preview panel) and `FormField` (generic input wrapper) are co-located sub-components.

### PDF Export Strategy
PDF generation uses dynamic imports (`await import('jspdf')` / `await import('html2canvas')`) to keep them out of the initial bundle. `html2canvas` captures `#invoice-preview` at 2× scale for high-res output; `jsPDF` converts it to multi-page A4.

### InvoicePreview — Inline Styles Only
`InvoicePreview` uses only inline styles (no Tailwind classes) because `html2canvas` does not reliably process Tailwind utility classes during canvas capture.

### Currency Formatting
All values use `Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 })` → `Rp 1.250.000`.

### Styling Approach
Tailwind CSS v4 handles layout/spacing. Custom classes in `styles.css` handle glassmorphism, gradient text, button variants, the toggle switch, and keyframe animations.
